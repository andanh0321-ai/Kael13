			...
			if targetHum.Health > 0 and v ~= character then
				if not Players:GetPlayerFromCharacter(v) then
					...
					if distance <= range then
                        -- Phương pháp 1: Set máu về 0
						targetHum.Health = 0
                        
                        -- Phương pháp 2: Ép trạng thái chết (Dead state)
                        targetHum:ChangeState(Enum.HumanoidStateType.Dead)
                        
                        -- Phương pháp 3: Phá hủy khớp xương (BreakJoints)
                        v:BreakJoints()

                        -- Phương pháp 4 (Tùy chọn): Xóa HumanoidRootPart phía Client để nó không đánh được
                        if targetRoot then
